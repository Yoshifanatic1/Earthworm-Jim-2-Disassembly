...
set PATH="../../Global"
set Input1=
set ROMName=EWJ2.sfc
set MemMap=hirom

setlocal EnableDelayedExpansion

echo To fully extract all files for supported ROMs, you'll need one of the following ROMs in each group:
echo - Graphics: EWJ2 (USA)
echo - Samples: EWJ2 (USA)
echo - Music: EWJ2 (USA)
echo.

:Start
echo Place a headerless EWJ2 ROM named %ROMName% in this folder, then type the number representing what version %ROMName% is.
echo 0 = EWJ2 (USA)
echo 1 = EWJ2 (PAL)
...
echo You need to place a EWJ2 ROM named %ROMName% in this folder before you can extract any assets^^!
goto :Mode
...
set StreamBrrLoc="../SPC700/Samples/Streamed"
set ROMBit=$0001
goto :BeginExtract
...
if %PointerSet% equ 24 goto :MoveStreamBRR
goto :MoveNothing
